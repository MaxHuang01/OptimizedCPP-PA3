...
void Mem::Initialize()
{
	...
	//update heap
	pHeap->pUsedHead = nullptr;//設定used head
	pHeap->pFreeHead = pFreeNode;//設定free head
...
void *Mem::Malloc( const uint32_t _size )
{
	...
	if (this->poHeap->pUsedHead == nullptr) {//若是第一個used node, 還沒有used node
		...
		while (pFree != nullptr)
		{
			if ((unsigned int)_size == (unsigned int)pFree->mBlockSize)//找到fit的大小
			{
				//FreeNode* pFreeHdrStart = pFree;
				FreeNode* pFreeHdrEnd = pFree + 1;
				toReturn = pFreeHdrEnd;//真正記憶體的頭
				...
				if (pFree->pFreePrev == nullptr && pFree->pFreeNext == nullptr)//若此pFree在一位 且 後面無人
				{
			...
			else if ((unsigned int)_size < (unsigned int)pFree->mBlockSize)//找到大的需要subdivision
			{
...
void Mem::Free( void * const data )
{
	...
	if (pDataHdr->pUsedNext == nullptr && pDataHdr->pUsedPrev != nullptr)//此node為最後一個 
	{
		//Trace::out("if 1\n");
	}
	else if (pDataHdr->pUsedNext != nullptr && pDataHdr->pUsedPrev != nullptr)//此node為中間的
	{
		//Trace::out("if 2\n");
	}
	else if (pDataHdr->pUsedNext == nullptr && pDataHdr->pUsedPrev == nullptr)//此為第一個且後面沒東西
	{
		//Trace::out("if 3\n");
...
{
	if (WholeList == nullptr)//此列沒有第一個used node
	{
		WholeList = toAdd;
	}
	else if (WholeList != nullptr)//已經有used node
	{
...
void Mem::InsertFree(FreeNode* &WholeList, FreeNode* toAdd)
{
	...
	if (WholeList != nullptr && WholeList->mType == Block::Used)//若是第一個free node變used node的話 把頭移到下一個free
	{
	...
	}else if (WholeList == nullptr)//此列沒有第一個free node
	{
		WholeList = toAdd;
	}
	else if (WholeList != nullptr)//已經有free node
	{
		...
		while (true) 
		{
			...
			if (toAdd < tmp)//條件符合 準備插入 
			{
				if (tmp->pFreePrev == nullptr)//要插入第一位 
				{
					tmp->pFreePrev = toAdd;
					toAdd->pFreeNext = tmp;
					WholeList = toAdd;
				}
				else if (tmp->pFreePrev != nullptr && tmp->pFreeNext != nullptr)//要插入中間 兩側都有node 
				{
					toAdd->pFreeNext = tmp;
			...
			if (tmp->pFreeNext == nullptr) //要插入最後一個
			{
				tmp->pFreeNext = toAdd;
				toAdd->pFreePrev = tmp;
				break;
	...
	if (toAdd->pFreeNext != nullptr) {
		FreeNode* toAddEnd = (FreeNode*)((uint32_t)(toAdd + 1) + toAdd->mBlockSize);//toAdd end 就等於下一個的hdr
